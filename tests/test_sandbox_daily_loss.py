...
async def test_sandbox_daily_loss_check():
    ...
    # Mock config values
    with patch('trader_bot.strategy_runner.TRADING_MODE', 'PAPER'), \
         patch('trader_bot.strategy_runner.MAX_DAILY_LOSS', 500.0), \
         patch('trader_bot.strategy_runner.MAX_DAILY_LOSS_PERCENT', 3.0), \
         patch('trader_bot.strategy_runner.GeminiTrader', return_value=mock_bot):
