...
def test_stale_due_to_latency(runner):
    runner.ticker_max_latency_ms = 1000
    runner._monotonic = lambda: 10.0
    stale, detail = runner._is_stale_market_data(
...
def test_stale_due_to_age(runner):
    runner.ticker_max_age_ms = 4000
    runner._monotonic = lambda: 100.0
    data = {"price": 100, "_fetched_monotonic": 95.0, "_latency_ms": 100}
    stale, detail = runner._is_stale_market_data(data)
    assert stale is True
    assert detail.get("reason") == "age"


def test_fresh_market_data(runner):
    runner.ticker_max_age_ms = 5000
    runner.ticker_max_latency_ms = 2000
